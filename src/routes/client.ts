...
// Exclui definitivamente um cliente. Só é permitido quando o cliente não tem
// nenhum pedido vinculado (histórico de vendas não pode ser perdido); nesse
// caso o vendedor deve apenas inativar o cliente em vez de excluí-lo.
router.delete("/:id", async (req: any, res: any) => {
  ...
  try {
    ...
    const ordersCount = await prisma.order.count({ where: { clientId: Number(id) } });
    if (ordersCount > 0) {
      return res.status(409).json({
        message:
          "Este cliente já possui pedidos registrados e não pode ser excluído. Inative o cliente em vez de excluí-lo.",
      });
    }
    ...
    res.status(500).json({ message: "Erro ao excluir cliente. Tente novamente em alguns instantes." });
  }
});
